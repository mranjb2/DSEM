...
!
      ...
      IF (statistics) THEN
       ...
       ElSEIF (rms) THEN
         WRITE(plt_unit,*) ' VARIABLES = "x","y","z","uu","vv","ww","uv","uw","vw","pf","pd","td"'
       END IF   
!       ELSE
!        WRITE(plt_unit,*) ' VARIABLES = "x","y","z","p","u","v","w","rho","T"'
!       END IF
      ELSE
       WRITE(plt_unit,*) ' VARIABLES = "x","y","z","rho"'
      END IF
         ...
         IF (myid == 0 .and. ngridmpi(id,2) == 0) THEN
            ...
            IF (statis) THEN
             ...
             IF (rms) THEN 
               ...
               statsa%Q_av(:,:,:,7:9) = statsfluct(idl)%Q_fluct(:,:,:,3:5)
             END IF
            ENDIF
         ELSEIF (numprocs >1) THEN
            nsnode = ngridmpi(id,2)                      ! node from which one is sending
            ...
            DO ns=1,1
              IF (myid /= 0 .and. myid==nsnode) THEN
                 IF (rms) THEN 
                  ...
                  stats(idl)%Q_av(:,:,:,7:9) = statsfluct(idl)%Q_fluct(:,:,:,3:5)
                 END IF
...
!                        p, u, v, w, rho, temp
                   WRITE(plt_unit,40) x(n,m,l), y(n,m,l), z(n,m,l), rho
                  END DO
                END DO
              END DO
         END IF 
      ...
      IF ( scale )     THEN ! scale out the jacobians for movies
         DO nv = 1,neq+nav
            DO l = 1,d%ncg(3)
               DO m = 1,d%ncg(2)
                  DO n = 1,d%ncg(1)
                     Q(n+1,m+1,l+1,nv) = stats%Q_av(n,m,l,nv)*d%jacob(n,m,l)
                  END DO
               END DO
            END DO
         END DO
      ELSE
         DO nv = 1,neq+nav
            DO l = 1,d%ncg(3)
               DO m = 1,d%ncg(2)
                  DO n = 1,d%ncg(1)
                     Q(n+1,m+1,l+1,nv) = stats%Q_av(n,m,l,nv)
                  END DO
               END DO
            END DO
         END DO
