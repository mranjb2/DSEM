...
!///////////////////////////////////////////////////////////////////////////////
!
      SUBROUTINE Read_Stats(d,ngrid,stats)
!
!.......................................................................
!     Read average values from an external file
!.......................................................................
!
      USE domain_definition
      USE stats_definition
...
!
      ...
      TYPE (statfl)     :: stats(ngp)
!
      DOUBLE PRECISION  :: ave(ngp*((nx+1)*(ny+1)),2+neq+nav)
      DOUBLE PRECISION  :: x,y
      DOUBLE PRECISION  :: tol=0.0001d0
      INTEGER           :: ngrid, count
      INTEGER           :: l,m,n,id,i,nv
!
      if (myid==0) write(*,*) 'Reading data...'
      
      open(unit=46, file="list.txt")
      
      count = 0
      do i = 1, ngp*((nx+1)*(ny+1))
	    read(46,*,end=10) ave(i,1),ave(i,2),ave(i,3),ave(i,4 ),ave(i,5 ),ave(i,6), &
	                      ave(i,7),ave(i,8),ave(i,9),ave(i,10),ave(i,11)
	    count = count + 1
      end do
10    if (myid==0) write(*,*) 'Finished reading data.'
            
      close(46)
      
      if (myid==0) write(*,*) 'Updating the average values...'
!
      DO id = 1, ngrid
    	 
    	 if (myid==0) write(*,*) 'Node', id, 'from', ngrid
		 ...
		 DO n = 1,d(id)%ncg(3)
		   DO m = 1,d(id)%ncg(2)
			 DO l = 1,d(id)%ncg(1)
			   ...
			   do i = 1, count
			     if (abs(x-ave(i,1))<tol .and. abs(y-ave(i,2))<tol) then
			       do nv = 1, neq+nav
			         stats(id)%Q_av(l,m,n,nv) = ave(i,nv+2) * nsample
			       end do
			       exit
			     end if
			   end do
      ...
      if (myid==0) write(*,*) 'Finished updating the average values.'
!
      RETURN
